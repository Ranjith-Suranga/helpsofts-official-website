	...
	<p class="description description-wide fusion-menu-style">
		<label for="<?php echo $name . '-' . $item_id; ?>>">
			...
			<select id="<?php echo $name . '-' . $item_id; ?>" class="widefat edit-menu-item-target" name="<?php echo $name . '[' . $item_id . ']'; ?>">
				...
				<option value="fusion-button-medium" <?php selected( $item->fusion_menu_style,  'fusion-button-medium' ); ?>	><?php _e('Button Medium', 'Avada' ); ?></option>
				<option value="fusion-button-large"	<?php selected( $item->fusion_menu_style,  'fusion-button-large' ); ?>	><?php _e('Button Large', 'Avada' ); ?></option>
				<option value="fusion-button-xlarge" <?php selected( $item->fusion_menu_style,  'fusion-button-xlarge' ); ?>	><?php _e('Button xLarge', 'Avada' ); ?></option>
			</select>
		</label>
	</p>
	<p class="field-megamenu-icon description description-wide">
		<label for="edit-menu-item-megamenu-icon-<?php echo $item_id; ?>">
			<?php _e( 'Menu Icon (use full font awesome name)', 'Avada' ); ?>
...
if( ! class_exists( 'FusionCoreFrontendWalker' ) ) {
	class FusionCoreFrontendWalker extends Walker_Nav_Menu {
		...
		private function set_megamenu_max_width() {
			...
			$megamenu_max_width = (int) str_replace( 'px', '', Avada()->settings->get( 'megamenu_max_width' ) );
			$megmanu_width = 0;

			// Site width in px
			if ( strpos( Avada()->settings->get( 'site_width' ), 'px' ) !== false ) {
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if( $depth === 1 && $this->menu_megamenu_status == "enabled" ) {
				...
				if ( $depth === 0 &&
					 $args->has_children &&
					 Avada()->settings->get( 'menu_display_dropdown_indicator' ) &&
					 Avada()->settings->get( 'header_position' ) == 'Right'
				) {
