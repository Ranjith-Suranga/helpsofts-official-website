...
class sidebar_generator {
	...
	public static function edit_form(){
		...
		?>
			<div class="pyre_metabox_field">
					...
					<div class="pyre_desc">
						<label><?php _e('Select Sidebar 1:', 'Avada'); ?></label>
						<p><?php _e('Select sidebar 1 that will display on this page. Choose "No Sidebar" for full width.', 'Avada'); ?></p>
					</div>
						...
						for($i=0;$i<1;$i++){ ?>
							...
							if(is_array($sidebar_replacements) && !empty($sidebar_replacements)){
								foreach($sidebar_replacements as $sidebar){
									...
									if( $screen->post_type == 'post' && $screen->action != 'add' && is_array( $selected_sidebar_replacement[$i] ) && empty( $selected_sidebar_replacement[$i] ) ) {
										$selected_sidebar_replacement[$i] = '';
									}
									if($selected_sidebar_replacement[$i] == $sidebar['name']){
										...
										if($sidebar['name'] == 'Blog Sidebar' || $sidebar['name'] == 'Default Sidebar'){
											if( $selected_sidebar_replacement[$i] != '' && $screen->post_type == 'post' ) {
												$selected = 'selected';
											} else {
												$selected = '';
											}
			...
			<div class="pyre_metabox_field">
					<input name="sbg_edit" type="hidden" value="sbg_edit" />
					<div class="pyre_desc">
						<label><?php _e('Select Sidebar 2:', 'Avada'); ?></label>
						<p><?php _e('Select sidebar 2 that will display on this page. Sidebar 2 can only be used if sidebar 1 is selected.', 'Avada'); ?></p>
					</div>
					<div class="pyre_field">
					<?php
					global $wp_registered_sidebars;
	...
	*/
	public static function get_sidebar($name="0"){
		...
		if(!$name && $selected_sidebar != '' && $selected_sidebar != "0"){
