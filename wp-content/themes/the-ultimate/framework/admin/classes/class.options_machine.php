...
*/
			...
			switch ( $value['type'] ) {
				...
				case 'text':
					...
					} else if ( $value['id'] == 'header_tagline' ) {

						$t_value = esc_html( stripslashes($smof_data[$value['id']]) );
					} else {
				...
				case 'select':
					...
					foreach ($value['options'] as $select_ID => $option) {
						...
						// Special case for font weights
						if ( strpos( $value['id'], 'font_weight_' ) !== FALSE ) {
							$theValue = $select_ID;
						}
				...
				case 'button':
					...
					$output .= '<img src="' . get_template_directory_uri() . '/framework/admin/assets/images/ajax-loader.gif" alt="" style="margin-left: 5px; display: none;" />';
				break;
				// End Avada edit
				...
				case 'heading':
					...
					$header_id = str_replace( array( 'heading_', '_' ),  array( '', '' ), strtolower($value['id']));
					$jquery_click_hook = "of-option-" . $header_id;
					$menu .= '<li class="'. $header_id .'"><a title="'.  $value['name'] .'" href="#'.  $jquery_click_hook  .'"'. $icon .'>'. $value['name'] .'</a></li>';
