<?php get_header(); ?>
	...
	if( Avada()->settings->get( 'portfolio_global_sidebar' ) ) {
		if( Avada()->settings->get( 'portfolio_sidebar' ) != 'None' ) {
			$sidebar_1 = array( Avada()->settings->get( 'portfolio_sidebar' ) );
		} else {
			$sidebar_1 = '';
		}

		if( Avada()->settings->get( 'portfolio_sidebar_2' ) != 'None' ) {
			$sidebar_2 = array( Avada()->settings->get( 'portfolio_sidebar_2' ) );
		} else {
			$sidebar_2 = '';
		}
	}
	...
	if( ! $sidebar_exists ) {
		...
		$content_class = 'portfolio-one-sidebar';
		if(Avada()->settings->get( 'portfolio_sidebar_position' ) == 'Left') {
			$content_css = 'float:right;';
			$sidebar_css = 'float:left;';
			$sidebar_exists = true;
			$sidebar_left = 1;
	...
	if( Avada()->settings->get( 'portfolio_global_sidebar' ) ) {
		if( Avada()->settings->get( 'portfolio_sidebar' ) != 'None' ) {
			...
			if( Avada()->settings->get( 'portfolio_sidebar_position' ) == 'Right' ) {
				$content_css = 'float:left;';
				$sidebar_css = 'float:right;';
				$sidebar_left = 2;
			} else {
		...
		if( Avada()->settings->get( 'portfolio_sidebar' ) != 'None' && Avada()->settings->get( 'portfolio_sidebar_2' ) != 'None' ) {
			$double_sidebars = true;
		}
	} else {
		$sidebar_1 = '0';
