...
class FusionSC_OneFull {
	...
	public function __construct() {
		...
		add_filter( 'fusion_attr_one-full-shortcode', array( $this, 'attr' ) );
		add_filter( 'fusion_attr_one-full-shortcode-wrapper', array( $this, 'wrapper_attr' ) );
	...
	function render( $args, $content = '') {
		...
		$html = sprintf( '<div %s><div %s>%s</div></div>%s', FusionCore_Plugin::attributes( 'one-full-shortcode' ), FusionCore_Plugin::attributes( 'one-full-shortcode-wrapper' ), $inner_content, $clearfix );
	...
	function attr() {
		...
		$attr['class'] = 'fusion-one-full fusion-layout-column';
